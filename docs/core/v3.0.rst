...
which is a string of characters. The following constraints apply to
node names:

* must be a Unicode code point sequence
* must not be the empty string (``""``)
* must not include the character ``"/"``
* must not be a string composed only of period characters, e.g. ``"."`` or ``".."``
* must not start with the reserved prefix ``"__"``
* must not contain `control characters (Cc) <https://www.compart.com/en/unicode/category/Cc>`_
  or `format characters (Cf) <https://www.compart.com/en/unicode/category/Cf>`_
...
When using non-ASCII Unicode characters, we recommend to only use NFC-normalized
characters, as recommended by the
`Unicode Standard Annex # 31 – Normalization and Case <https://www.unicode.org/reports/tr31/#normalization_and_case>`_
