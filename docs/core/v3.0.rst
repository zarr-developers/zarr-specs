...
^^^^^^^^^^^^^^^^^^^
    ...
    is indicated by an empty string, ``""``.  Within a single zarr array, all
    non-empty dimension names must be distinct (no two dimensions may have the
    same non-empty name), but more than one dimension may have an empty name.
    This specification does not place any restrictions on the use of the same
    dimension name across multiple arrays within the same zarr hierarchy, but
    extensions or specific applications may do so.  If ``dimension_names`` is
