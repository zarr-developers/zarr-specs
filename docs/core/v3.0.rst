...
See `protocol extensions`_ below.

Stability Policy
----------------

This core protocol specification adheres to adapted Semantic Versioning regarding
read and write compatibility using the ``MAJOR.MINOR[.PATCH]?`` version
number format, where the patch number is optional and implicitly ``0``.
A zarr implementation provides the read and write API by implementing
this specification, the datasets are considered consumers in this case.

Notably, this excludes extensions, codecs and stores from the compatibility
of the core protocol. However, versioned extensions and stores are also
expected to follow this stability policy.

This means that implementations based on a ``3.X`` specification will be
able to read and write to datasets that follow a ``3.Y`` specification
where ``Y <= X``, as long as it only uses
...
^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    the protocol extension and dereferences to a human-readable representation
    of the specification.  The ``type`` is required and the value is
    defined by the protocol extension. The
    object may also contain a ``configuration`` object which consists of the
...
Many types of extensions can exist for a Zarr protocol, they can be grouped
as following:
