...
.. note::
    Node names starting with an underscore will be prefix with an additional
    underscore in the path. This avoids conflicts with zarr-internal keys
    which use a single underscore as a prefix.
