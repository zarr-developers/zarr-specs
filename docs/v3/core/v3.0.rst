...
^^^^^^^^^^^^^^
    ...
    IEEE 754 floating point numbers (``float{16,32,64}``)
      ...
      - A JSON string of the form:
        ...
        - ``"inf"``, denoting positive infinity;
        - ``"-inf"``, denoting negative infinity;
        - ``"nan"`` or ``"-nan"``, equivalent to ``"nan(0x1)"`` and
          ``"-nan(0x1)"`` respectively;
        - ``"nan(0xYYYY)"`` or ``"-nan(0xYYYY)"``, specifying a NaN value with
          positive or negative sign bit and payload indicated by ``YYYY``, where
          ``YYYY`` is the base-16 representation of a *non-zero* integer
          indicating the NaN payload.  The NaN payload ``YYYY``` specifies the
          mantissa bits in the NaN representation in reverse order; that is, the
          least significant bit of ``0xYYYY`` is the most significant bit of the
          mantissa.  According to IEEE 754-2019, a quiet NaN has a payload with
          a least significant bit of 1 (most significant bit of the mantissa
          equal to 1), while a signaling NaN has a payload with a least
          significant bit of 0 (most significant bit of the mantissa equal to
          0).  This is consistent with x86 and ARM, but not consistent with MIPS
          where the behavior may depend on a flag.
    ...
    Complex numbers (``complex{64,128}``)
      ...
      For example, ``[1, 2]`` indicates ``1 + 2i`` and ``["-inf", "nan(0x7)"]``
      indicates a complex number with real component of -inf and imaginary
      component of positive NaN with payload of ``0x7``.
