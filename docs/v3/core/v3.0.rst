...
- v3 has a different metadata schema to accommodate the extension points.

- v3 introduces storage transformers, which transform reads and writes between
  array and store.

- v3 introduces chainable codecs to unify the codec and filter concepts of v2.

- The default chunk keys have changed from `0.1.2` to `c/0/1/2`. The v2-style
  chunk keys are still available through the ``v2`` ``chunk_key_encoding``.
