...
A Zarr group metadata document is a [RFC8259]_ JSON document
containing configuration metadata for a Zarr group. In this section,
...
a set of name/value pairs. @@TODO figure out how to refactor this
content with the array metadata section to avoid duplication.@@

A Zarr group metadata document must contain a single object, here
...
`zarr_format`
    ...
    metadata format. For Zarr groups conforming to this specification,
    the value must be the string
    "http://purl.org/zarr/spec/protocol/core/3.0".

    @@TODO figure out how to refactor this content with array metadata
    to avoid duplication.@@

`extensions`

    Provides protocol extensions which define additional features used
    by the Zarr group or other modifications to the metadata format.
