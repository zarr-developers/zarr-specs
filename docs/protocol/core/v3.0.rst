...
fallback types of respectively 3 and 11 bytes longs, and implementation should
ensure to pass or accept those types unchanged when using extension fallback.
For array whose total length is not a multiple of 8, zarr implementation can
expect expects a buffer which length is a multiple of 8bits, padded with
...
``metadata_encoding``
    ...
    A string containing the URI that has been used for encoding
    group and array metadata documents. If JSON encoding is used, this
    value must be ``"https://purl.org/zarr/spec/metadata/json/3.0"``.
...
which do not contain the character "/" after the given prefix.
    ...
    Thus we know that path1, and path2 are terminal objects with data, and that
    ``path3`` and ``path4``


    Similar to ``ls -p``

    Note: In practice I believe this means that this means keys always ends in
    ``/``, ``.json``, ``.array``, ``.group``, or otherwise they will be chunks
    data.
