...
listing keys and accessing metadata can be a expensive – high latency
– operation. Thus is a nested hierarchy listing all available groups, datasets
and chunks can be a time consuming operation.

The v3 spec thus tried to separate the metadata, from group and dataset data
...
not exists. This allow lock-free write operation for non-contending
...
One of the Non-goal of Zarr Spec V3 is to cover all use cases, and to provide a
path forward for extensibility and future standardisation of extensions without
the need to rely on the Zarr core team. A challenge is to make sure
implementations of the Zarr protocol for which used extension are not available
can still give user access to data without triggering corruption when possible.
