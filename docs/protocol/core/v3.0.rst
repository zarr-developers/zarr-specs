...
Deletion of an implicit group may automatically delete any empty parent. For
example on a S3 store where the namespace is flat, deletion of the last key with
a prefix will delete all the implicit group in the prefix.

Care must thus be taken when deleting a array or a group if the parent needs to
