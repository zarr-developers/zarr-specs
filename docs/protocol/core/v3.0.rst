...
which consists of three types metadata documents: a entry point metadata document,
...
Each Zarr hierarchy must have a entry point metadata document, which
...
For example, below is a entry point metadata document, specifying that
...
For example, below is a entry point metadata document as above, but also
...
create a store implementation spec and contribute it to the
`zarr-specs GitHub repository <https://github.com/zarr-developers/zarr-specs/>`_.
