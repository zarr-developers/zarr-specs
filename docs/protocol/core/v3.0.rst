...
which consists of three types metadata documents: an entry point metadata document,
array metadata documents, and group metadata documents. Each type of metadata
document is described in the following subsections.
...
``fill_value``
    ...
    @@TODO what about floating point data types?
    @@TODO what about nan and inf?
    @@TODO what about extension data types?
...
names as defined above in the `Array metadata`_ section. All other names
are reserved for future versions of this specification. See also the section
...
The entry point metadata document must be encoded as JSON. The array and
...
which do not contain the character "/" after the given prefix.
   ...
   @@TODO@@ more general list with depth XXX
