...
**writeable**, **erasable** and **listable**. It is not necessary for a store
...
A **erasable stores** support the following operations:
...
.. note::
   ...
   Some KV stores do allows creations and update of keys, but not deletion.
   For example Zip archive do not allow removal of content without recreating
   the full archive.

   Unability to delete can affect ability to rename keys as well, as a ranema as
   often a succession or an atomic combinaison of deleting data and setting it
   to a new name.
